...
private:
    ...
    cv::Mat bgr_image_, hsv_image_, depth_image, mask_;
    cv::Ptr<cv::CLAHE> clahe_;              // 直方图均衡化
    TargetFeature last_feature_;            // 上一次目标特征
    cv::Mat last_hsv_roi_;                  // 缓存上一次HSV区域
...
public:
    ...
    {
        // 初始化订阅者
        rgb_sub_.subscribe(nh_, "/color/image_raw", 10, ros::TransportHints().tcpNoDelay());
        depth_sub_.subscribe(nh_, "/depth/image_raw", 10, ros::TransportHints().tcpNoDelay());
        sync_.reset(new Synchronizer<SyncPolicy>(SyncPolicy(200), rgb_sub_, depth_sub_));
