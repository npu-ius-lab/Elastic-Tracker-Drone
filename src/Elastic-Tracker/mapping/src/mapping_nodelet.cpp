...
class Nodelet : public nodelet::Nodelet {
 private:
  ...
  ros::Subscriber extrinstic_sub_;
  ...
  double mask_x_ = 0.3, mask_y_ = 0.3, mask_z_ = 1.0;
  ...
  void depth_odom_callback(const sensor_msgs::ImageConstPtr& depth_msg,
    ...
    if (!have_extrinstic_){
      ROS_WARN_THROTTLE(1.0, "[mapping]Waiting for camera extrinsic...");
      return;
    }
    ...
    Eigen::Vector3d cam_p = body_q * extrinsic_t_ + body_p;
    Eigen::Quaterniond cam_q = body_q * extrinsic_r_;

    //ROS_INFO_THROTTLE(1.0, "Cam P&Q is OK!");
  ...
  void init(ros::NodeHandle& nh) {
    ...
    nh.getParam("mask_x", mask_x_);
    nh.getParam("mask_y", mask_y_);
    nh.getParam("mask_z", mask_z_);
    ...
    if (use_global_map_) {
      ...
      extrinstic_sub_ = nh.subscribe<nav_msgs::Odometry>("extrinsic", 1, &Nodelet::extrinsic_callback, this);
    }

    if (use_mask_) {
      target_odom_sub_ = nh.subscribe<nav_msgs::Odometry>("target", 1, &Nodelet::target_odom_callback, this, ros::TransportHints().tcpNoDelay());
    }
    ROS_WARN("Mapping node initialized!");
  }
