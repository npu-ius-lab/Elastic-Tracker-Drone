...
class Nodelet : public nodelet::Nodelet {
 private:
  ...
  ros::Subscriber gridmap_sub_, odom_sub_, target_sub_, triger_sub_, land_triger_sub_, px4_state_sub_;
  ...
  double tracking_dur_, tracking_dist_, tolerance_d_, tolerance_yaw_;
  ...
  std::atomic_bool is_offboard_ = ATOMIC_VAR_INIT(false);
  ...
  void PX4State_Callback(const mavros_msgs::State::ConstPtr &msg)
  {
    bool is_connected = msg->connected;
    bool is_armed = msg->armed;
    // PX4未连接或未解锁，拒绝执行
    if (!is_connected || !is_armed)
      return;
    // 判断是否在OFFBOARD状态
    if (msg->mode == "OFFBOARD") {
      ...
      is_offboard_ = false; 
      target_received_ = false;   // 退出offboard模式以后，默认未收到目标（需要目标重新触发）
    }
  }
  ...
  void plan_timer_callback(const ros::TimerEvent& event) {
    ...
    // 发布心跳包以供traj_server确认
    heartbeat_pub_.publish(std_msgs::Empty());

    // 确认是否有odom
    if (!odom_received_) {
      ROS_INFO_THROTTLE(1.0,"No Odom!");
      return;
    }
    if (!map_received_){
      ROS_INFO_THROTTLE(1.0,"No Map!");
      return;
    }
    ...
    odom_lock_.clear();
    Eigen::Vector3d odom_p(odom_msg.pose.pose.position.x,             // 里程计位置
                           odom_msg.pose.pose.position.y,
                           odom_msg.pose.pose.position.z);
    Eigen::Vector3d odom_v(odom_msg.twist.twist.linear.x,             // 里程计速度
                           odom_msg.twist.twist.linear.y,
                           odom_msg.twist.twist.linear.z);
    Eigen::Quaterniond odom_q(odom_msg.pose.pose.orientation.w,       // 里程计位姿
                              odom_msg.pose.pose.orientation.x,
                              odom_msg.pose.pose.orientation.y,
                              odom_msg.pose.pose.orientation.z);

    Eigen::Vector3d project_yaw = odom_q.toRotationMatrix().col(0);   // 里程计偏航角
    double now_yaw = std::atan2(project_yaw.y(), project_yaw.x());

    // 确认是否在offboard模式
    // 如是，则发布悬停指令，并置悬停标志位为真
    if (!is_offboard_){
      pub_hover_p(odom_p, now_yaw, ros::Time::now());
      wait_hover_ = true;
      ROS_INFO_THROTTLE(1.0,"PX4 is Not in offboard mode. Ready to hover.");
      return;
    }
    ...
    if (land_triger_received_) {
      if (std::fabs((target_p - odom_p).norm() < 0.1 && odom_v.norm() < 0.1 && target_v.norm() < 0.2)) {
        ...
        ROS_WARN_THROTTLE(1.0,"[planner] HOVERING...");
        return;
      }
    ...
    bool generate_new_traj_success = prePtr_->predict(target_p, target_v, target_predict);
    ...
    if (generate_new_traj_success) {
      ...
      visPtr_->visualize_path(target_predict, "car_predict");
    ...
    if (generate_new_traj_success) {
      ...
      if (land_triger_received_) {
        ...
        generate_new_traj_success = envPtr_->findVisiblePath(p_start, target_predict, way_pts, path);
      }
    ...
    if (generate_new_traj_success) {
      ...
      if (land_triger_received_) {
        ...
        envPtr_->generate_visible_regions(target_predict, way_pts,
                                          visible_ps, thetas);
        // ros::Time t_end1 = ros::Time::now();
        // t_path += (t_end1 - t_front1).toSec() * 1e3;
        visPtr_->visualize_pointcloud(visible_ps, "visible_ps");
        visPtr_->visualize_fan_shape_meshes(target_predict, visible_ps, thetas, "visible_region");
      ...
      if (land_triger_received_) {
        ...
        generate_new_traj_success = trajOptPtr_->generate_traj(iniState, finState, target_predict, hPolys, traj);
      } 
    ...
    if (valid) {
      force_hover_ = false;
      ROS_WARN_THROTTLE(1.0,"[planner] REPLAN SUCCESS");
    ...
    // 强制悬停
      ...
      ROS_ERROR_THROTTLE(1.0,"[planner] REPLAN FAILED, HOVERING...");
      replanStateMsg_.state = 1;
      replanState_pub_.publish(replanStateMsg_);
      return;
    } 
    ...
    // 紧急停止
      ...
      force_hover_ = true;
      ROS_FATAL_THROTTLE(1.0,"[planner] EMERGENCY STOP!!!");
      replanStateMsg_.state = 2;
      replanState_pub_.publish(replanStateMsg_);
      pub_hover_p(iniState.col(0), now_yaw, replan_stamp);
      return;
    } 
    
    // 执行最后规划的轨迹
    else {
      ROS_ERROR_THROTTLE(1.0,"[planner] REPLAN FAILED, EXECUTE LAST TRAJ...");
  ...
  /*void fake_timer_callback(const ros::TimerEvent& event) {
    ...
    if ((goal_ - odom_p).norm() < tracking_dist_ + tolerance_d_ && odom_v.norm() < 0.1) {
      if (!wait_hover_) {
        pub_hover_p(odom_p, 0, ros::Time::now());
        wait_hover_ = true;
      }
    ...
    if (valid) {
      ...
      replanStateMsg_.state = 2;
      replanState_pub_.publish(replanStateMsg_);
      pub_hover_p(iniState.col(0), 0, replan_stamp);
      return;
    } else {
  ...
  void init(ros::NodeHandle& nh) {
    ...
    nh.getParam("tolerance_yaw", tolerance_yaw_);
    ...
    px4_state_sub_ = nh.subscribe<mavros_msgs::State>("PX4_state", 1, &Nodelet::PX4State_Callback, this, ros::TransportHints().tcpNoDelay());
